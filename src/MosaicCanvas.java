...
public class MosaicCanvas extends Canvas {
    ...
    public Color[][] grid; // An array that contains the rectangles' colors.
       ...
       new Thread(new Runnable() {
           ...
           public void run() {
               ...
               for (int i = 0; i < getRowCount(); i++){
                   ...
                   for (int i = 0; i < getColumnCount(); i++) {
                       for (int i2 = 0; i2 < getRowCount(); i2++) {
                           int x = randInt(0, getRowCount());
                           int y = randInt(0, getColumnCount());
                           int colorrand = randInt(0, randomColors.size() - 1);
                           setColor(x, y, randomColors.get(colorrand));
               ...
               for (int i = 0; i < ypositions.size(); i++){
                   for (int i2 = 0; i2 < xpositions.size(); i2++){
                       Color color = getColor(i2, i);
                       if (color != Color.GREEN){
                           setColor(i2, i, Color.GREEN);
                       }
                   }
               }
               randomColor();
